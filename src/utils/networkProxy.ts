 ...
 *
 * Provides proxy configuration for all outbound HTTP/HTTPS requests from the Piramyd Agent extension.
...
async function configureUndiciProxy(config: ProxyConfig): Promise<void> {
	...
	try {
		...
		// To ensure Piramyd Agent's `fetch()` calls are proxied, patch global fetch in debug mode.
