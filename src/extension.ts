...
/**
 * Check if we should auto-open the Piramyd Agent sidebar after switching to a worktree.
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening Piramyd Agent sidebar for worktree: ${worktreeAutoOpenPath}`)

			// Open the Piramyd Agent sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize the provider *before* the Piramyd Agent Cloud service.
	const provider = new ClineProvider(context, outputChannel, "sidebar", contextProxy, mdmService)

	// Initialize Piramyd Agent Cloud service.
